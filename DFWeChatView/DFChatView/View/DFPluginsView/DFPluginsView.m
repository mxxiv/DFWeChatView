...
-(void) addPluginsToContentView:(UIView *) contentView index:(NSUInteger)index
{
    ...
    for (unsigned long i = index * _pluginsEveryPage; counter<_pluginsEveryPage && i < _plugins.count; i++) {
        ...
        if (counter > 3) {
            x = horizontalSpace + (counter-4)*(horizontalSpace + PluginViewWidth);
        }
        ...
        UIButton *pluginBtn = [self getPluginView:CGRectMake(x, y, width, height) icon:plugin.icon name:plugin.name];
...
-(UIButton *) getPluginView:(CGRect) frame  icon:(NSString *) icon name:(NSString *)name
{
    ...
    imageView.layer.borderColor = [UIColor colorWithWhite:240/255.0 alpha:1.0].CGColor;
