...
-(void) sendMessage
{
    if (_delegate != nil && [_delegate conformsToProtocol:@protocol(DFInputToolbarViewDelegate)]  && [_delegate respondsToSelector:@selector(onMessageSend:)]) {
        if (![_inputTextView.text isEqualToString:@""]) {
            [_delegate onMessageSend: _inputTextView.text ];
            _inputTextView.text =@"";
            _lines = 1;
        }
